...
def compare_models(prompt: str) -> dict:
    ...
    """
    gpt4o_text, gpt4o_latency = call_openai(prompt, model=OPENAI_MODEL)
    mini_text, mini_latency = call_openai_mini(prompt)

    est_tokens = len(gpt4o_text.split()) / 0.75
    gpt4o_cost_estimate = (
        (est_tokens / 1000.0)
        * PRICING_PER_1K_TOKENS.get(OPENAI_MODEL, PRICING_PER_1K_TOKENS["gpt-4o"])["output"]
    )
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    lines = []
    header = f"{'Prompt':<30} | {'GPT-4o Response':<30} | {'Mini Response':<30} | {'GPT-4o Latency':<15} | {'Mini Latency':<15}"
    lines.append(header)
    lines.append("-" * len(header))

    for item in results:
        p = item.get("prompt", "")[:27] + "..." if len(item.get("prompt", "")) > 30 else item.get("prompt", "")
        r4 = item.get("gpt4o_response", "")[:27] + "..." if len(item.get("gpt4o_response", "")) > 30 else item.get("gpt4o_response", "")
        rm = item.get("mini_response", "")[:27] + "..." if len(item.get("mini_response", "")) > 30 else item.get("mini_response", "")
        l4 = f"{item.get('gpt4o_latency', 0.0):.2f}s"
        lm = f"{item.get('mini_latency', 0.0):.2f}s"
        lines.append(f"{p:<30} | {r4:<30} | {rm:<30} | {l4:<15} | {lm:<15}")
